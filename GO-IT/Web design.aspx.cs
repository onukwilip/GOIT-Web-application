    ...
    public partial class Web_design : System.Web.UI.Page
    {
        ...
        protected void Submit(object sender, EventArgs e)
        {
            try
            {
                ...
                string id = "002", client = _ID != null ? _ID.Value.Split('=')[1] : "undefined", orderid = new GeneralClass()._random().ToString(), Mydesc = description.Value, Myfeatures = features.Value, _Myfunc = functionality.Value, _MyData = database.Value, ext = Path.GetExtension(zipfile.FileName);
                    ...
                    if (_read.Read() == true)
                    {
                        ...
                        if (read.Read() == true)
                        {
                            ...
                            else if (extradata.Checked == true && _MyData.Length < 50)
                            {
                                ...
                                errorDbase.InnerHtml = "*Contents description must not be less than 50 characters...";
                            }
                            ...
                            else
                            {
                                ...
                                /*  if (hosting.Checked == true)
                                  {
                                      string extra_insert = "INSERT INTO CartExtra(Name, ID, OrderID, Price) VALUES('" + hosting.Value + "', '" + new GeneralClass()._random().ToString() + "', '" + orderid + "', " + Convert.ToInt32(host_price.InnerHtml) + ")";
                                      SqlCommand extra_cmd = new SqlCommand(extra_insert, con);
                                      SqlDataAdapter extra_sda = new SqlDataAdapter();
                                      extra_sda.InsertCommand = extra_cmd;
                                      extra_sda.InsertCommand.ExecuteNonQuery();
                                ...
                                Response.Redirect("Web design.aspx");
                            }
